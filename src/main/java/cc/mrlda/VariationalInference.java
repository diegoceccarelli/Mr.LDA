...
/**
 * This is the entry point of vanilla MapReduce latent Dirichlet allocation
 * package.
 * 
 * @author kzhai
 */
public class VariationalInference extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		// Appender loggingAppender = new WriterAppender(new TTCCLayout(), new
		// FileOutputStream(
		// VariationalInference.class.getSimpleName() + ".log." +
		// variationalOptions.toString() + "."
		...
		// return run(configuration, inputPath, outputPath, numberOfTopics,
		// numberOfTerms,
		// numberOfIterations, mapperTasks, reducerTasks, localMerge, training,
		// randomStartGamma,
		// resume, informedPrior, modelPath, snapshotIndex, directEmit,
		// truncateBeta);
	}

	// private int run(Configuration configuration, VariationalOptions
	// variationalOptions, String
	// inputPath, String outputPath,
	// int numberOfTopics, int numberOfTerms, int numberOfIterations, int
	// mapperTasks,
	// int reducerTasks, boolean localMerge, boolean training, boolean
	// randomStartGamma,
	...
	private int run(Configuration configuration,
			VariationalInferenceOptions variationalOptions) throws Exception {
		...
		if (informedPrior != null) {
			...
			Preconditions.checkArgument(
					fs.exists(informedPrior) && fs.isFile(informedPrior),
		...
		Path documentGlobDir = new Path(tempDir.toString() + Path.SEPARATOR
				+ Settings.GAMMA + Settings.UNDER_SCORE + Settings.GAMMA
		...
		String betaGlobDir = tempDir.toString() + Path.SEPARATOR
				+ Settings.BETA + Settings.UNDER_SCORE + Settings.BETA
				+ Settings.DASH + Settings.STAR;

		String alphaPath = outputPath + Settings.ALPHA + Settings.DASH;
		Path alphaSufficientStatisticsDir = new Path(tempDir.toString()
				+ Path.SEPARATOR + "part-00000");
		double[] alphaVector = new double[numberOfTopics];

		if (!training) {
			alphaDir = new Path(modelPath + Settings.ALPHA + Settings.DASH
					+ snapshotIndex);
			betaDir = new Path(modelPath + Settings.BETA + Settings.DASH
					+ snapshotIndex);
		} else {
			if (!resume) {
				// initialize alpha vector randomly - if it doesn't already
				// exist
				...
				try {
					sequenceFileWriter = new SequenceFile.Writer(fs, conf,
							alphaDir, IntWritable.class, DoubleWritable.class);
				...
				inputDir = new Path(outputPath + Settings.GAMMA + Settings.DASH
						+ snapshotIndex);
			}
		}
		...
		do {
			...
			if (training) {
				conf.setJobName(VariationalInference.class.getSimpleName()
			...
			if (iterationCount != 0) {
				Preconditions.checkArgument(fs.exists(betaDir),
						"Missing model parameter beta...");
				DistributedCache.addCacheFile(betaDir.toUri(), conf);
			}
			Preconditions.checkArgument(fs.exists(alphaDir),
					"Missing model parameter alpha...");
			DistributedCache.addCacheFile(alphaDir.toUri(), conf);

			if (informedPrior != null) {
				Preconditions.checkArgument(fs.exists(informedPrior),
						"Informed prior does not exist...");
			...
			// conf.setFloat(Settings.PROPERTY_PREFIX +
			// "model.mapper.converge.likelihood",
			// Settings.DEFAULT_GAMMA_UPDATE_CONVERGE_CRITERIA);
			conf.setInt(Settings.PROPERTY_PREFIX
					+ "model.mapper.converge.iteration",
					Settings.MAXIMUM_LOCAL_ITERATION);

			conf.setInt(Settings.PROPERTY_PREFIX + "model.topics",
					numberOfTopics);
			conf.setInt(Settings.PROPERTY_PREFIX + "corpus.terms",
					numberOfTerms);
			conf.setBoolean(Settings.PROPERTY_PREFIX + "model.train", training);
			conf.setBoolean(Settings.PROPERTY_PREFIX + "model.random.start",
					randomStartGamma);
			conf.setBoolean(Settings.PROPERTY_PREFIX + "model.informed.prior",
					informedPrior != null);
			conf.setBoolean(Settings.PROPERTY_PREFIX
					+ "model.mapper.direct.emit", directEmit);
			conf.setBoolean(Settings.PROPERTY_PREFIX + "model.truncate.beta",
					truncateBeta);
			...
			if (training) {
				MultipleOutputs.addMultiNamedOutput(conf, Settings.BETA,
						SequenceFileOutputFormat.class, PairOfIntFloat.class,
						HMapIDW.class);
				// MultipleOutputs.addMultiNamedOutput(conf, Settings.BETA,
			...
			try {
				...
				sLogger.info("Iteration " + (iterationCount + 1)
						+ " finished in "
						+ (System.currentTimeMillis() - startTime) / 1000.0
						+ " seconds");

				Counters counters = job.getCounters();
				double logLikelihood = -counters.findCounter(
						ParameterCounter.LOG_LIKELIHOOD).getCounter()
						* 1.0 / Settings.DEFAULT_COUNTER_SCALE;
				sLogger.info("Log likelihood of the model is: " + logLikelihood);

				numberOfDocuments = (int) counters.findCounter(
						ParameterCounter.TOTAL_DOCS).getCounter();
				sLogger.info("Total number of documents is: "
						+ numberOfDocuments);
				numberOfTerms = (int) (counters.findCounter(
						ParameterCounter.TOTAL_TERMS).getCounter() / numberOfTopics);
				sLogger.info("Total number of terms is: " + numberOfTerms);

				double configurationTime = counters.findCounter(
						ParameterCounter.CONFIG_TIME).getCounter()
						* 1.0 / numberOfDocuments;
				sLogger.info("Average time elapsed for mapper configuration (ms): "
						+ configurationTime);
				double trainingTime = counters.findCounter(
						ParameterCounter.TRAINING_TIME).getCounter()
						* 1.0 / numberOfDocuments;
				sLogger.info("Average time elapsed for processing a document (ms): "
						+ trainingTime);
				...
				try {
					...
					sequenceFileReader = new SequenceFile.Reader(fs, alphaDir,
							conf);
					alphaVector = importAlpha(sequenceFileReader,
							numberOfTopics);
					sLogger.info("Successfully import old alpha vector from file "
							+ alphaDir);

					// load alpha sufficient statistics into the system
					double[] alphaSufficientStatistics = null;
					sequenceFileReader = new SequenceFile.Reader(fs,
							alphaSufficientStatisticsDir, conf);
					alphaSufficientStatistics = importAlpha(sequenceFileReader,
							numberOfTopics);
					...
					if (symmetricAlpha) {
						...
						double newAlpha = updateScalarAlpha(numberOfTopics,
								numberOfDocuments, oldAlpha,
								totalAlphaSufficientStatistics);
						for (int i = 0; i < numberOfTopics; i++) {
							alphaVector[i] = newAlpha;
						}
					} else {
						alphaVector = updateVectorAlpha(numberOfTopics,
								numberOfDocuments, alphaVector,
					...
					sequenceFileWriter = new SequenceFile.Writer(fs, conf,
							alphaDir, IntWritable.class, DoubleWritable.class);
					exportAlpha(sequenceFileWriter, alphaVector);
					sLogger.info("Successfully export new alpha vector to file "
							+ alphaDir);
				} finally {
				...
				if (localMerge) {
					throw new IOException(
							"Please disable local merge option...");
					// betaDir = FileMerger.mergeSequenceFiles(betaGlobDir,
					// betaPath + (iterationCount + 1),
					// 0,
					...
					c.set("mapred.job.queue.name",
							variationalOptions.getQueue());

					betaDir = FileMerger.mergeSequenceFiles(c, betaGlobDir,
							betaPath + (iterationCount + 1), reducerTasks,
							PairOfIntFloat.class, HMapIDW.class, true, true);
					// betaDir = FileMerger.mergeSequenceFiles(betaGlobDir,
					// betaPath + (iterationCount + 1),
					// reducerTasks, PairOfIntFloat.class, ProbDist.class, true,
					// true);
					// betaDir = FileMerger.mergeSequenceFiles(betaGlobDir,
					// betaPath + (iterationCount + 1),
					// reducerTasks, PairOfIntFloat.class, BloomMap.class, true,
					// true);
					// betaDir = FileMerger.mergeSequenceFiles(betaGlobDir,
					// betaPath + (iterationCount + 1),
					// reducerTasks, PairOfIntFloat.class, HashMap.class, true,
					// true);
				}
				...
				if (!randomStartGamma) {
					gammaDir = inputDir;
					inputDir = new Path(outputPath + Settings.GAMMA
							+ Settings.DASH + (iterationCount + 1));

					// TODO: technically, can rename the entire directory at
					// this point of time, but found out
					// there are a lot of "part-*" files left-over, and set the
					// outputformat to
					// NullOutputFormat does not resolve this problem, hence,
					...
					for (FileStatus file : fileStatus) {
						Path newPath = new Path(inputDir.toString()
				...
				sLogger.info("Log likelihood after iteration "
						+ (iterationCount + 1) + " is " + logLikelihood);
				if (Math.abs((lastLogLikelihood - logLikelihood)
						/ lastLogLikelihood) <= Settings.DEFAULT_GLOBAL_CONVERGE_CRITERIA) {
					sLogger.info("Model converged after "
				...
				if (training) {
					...
					FileStatus[] ls = fs.globStatus(new Path(tempDir,"gamma*"));
					for (FileStatus file : ls) {
						fs.rename(file.getPath(),new Path(outputDir,file.getPath().getName()));
					}
	...
	/**
	 * This method updates the hyper-parameter alpha vector of the topic
	 * Dirichlet prior, which is an asymmetric Dirichlet prior.
	 * 
	 ...
	 *            the alpha sufficient statistics collected from the corpus
	 * @return
	 */
	public static double[] updateVectorAlpha(int numberOfTopics,
			int numberOfDocuments, double[] alphaVector,
			...
			while (keepGoing) {
				...
				for (int i = 0; i < numberOfTopics; i++) {
					...
					alphaGradientVector[i] = numberOfDocuments
							* (Gamma.digamma(alphaSum) - Gamma
									.digamma(alphaVector[i]))
							+ alphaSufficientStatistics[i];

					// compute alphaHessian
					alphaHessianVector[i] = -numberOfDocuments
				...
				while (true) {
					...
					for (int i = 0; i < numberOfTopics; i++) {
						double stepSize = Math.pow(
								Settings.DEFAULT_ALPHA_UPDATE_DECAY_FACTOR,
								decay)
				...
				for (int j = 0; j < numberOfTopics; j++) {
					...
					if (Math.abs((alphaVectorUpdate[j] - alphaVector[j])
							/ alphaVector[j]) >= Settings.DEFAULT_ALPHA_UPDATE_CONVERGE_THRESHOLD) {
						keepGoing = true;
					}
				}
	 ...
	 *            the reader for a sequence file
	 * @param numberOfTopics
	 *            number of topics
	 * @return
	 * @throws IOException
	 */
	public static double[] importAlpha(SequenceFile.Reader sequenceFileReader,
			int numberOfTopics) throws IOException {
		...
		while (sequenceFileReader.next(intWritable, doubleWritable)) {
			Preconditions.checkArgument(
					intWritable.get() > 0
							&& intWritable.get() <= numberOfTopics,
					"Invalid alpha index: must be an integer in (0, "
							+ numberOfTopics + "]...");
		...
		Preconditions.checkArgument(counts == numberOfTopics,
				"Invalid alpha vector...");
	 ...
	 *            the writer for sequence file
	 * @param alpha
	 *            the alpha vector get exported
	 * @throws IOException
	 */
	 ...
	 * @return
	 */
	public static double updateScalarAlpha(int numberOfTopics,
			int numberOfDocuments, double alphaInit,
			double alphaSufficientStatistics) {
		int alphaUpdateIterationCount = 0;
			...
			while (keepGoing) {
				...
				alphaGradient = numberOfDocuments
						* (numberOfTopics * Gamma.digamma(alphaSum) - numberOfTopics
								* Gamma.digamma(alphaUpdate))
						+ alphaSufficientStatistics;

				// compute alphaHessian
				alphaHessian = numberOfDocuments
						* (numberOfTopics * numberOfTopics
								* Gamma.trigamma(alphaSum) - numberOfTopics
